...
{
	if (snapshotables.find(cuerpo->obtenerID()) != snapshotables.end())
...
void Mundo::actualizar(real segundosDesdeUltima){
	...
	//megamans
	std::map<uint, Megaman*>::const_iterator jit;
	for(jit=megamanes.begin(); jit!=megamanes.end(); ++jit){
		(jit->second)->actualizar(segundosDesdeUltima);
	}
}
